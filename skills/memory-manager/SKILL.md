...
### Search memories
```bash
cd ${CLAUDE_SKILL_DIR} &&node src/cli.js search "<query>"
```

### Add to long-term memory
```bash
cd ${CLAUDE_SKILL_DIR} &&node src/cli.js add "<text>"
```

### Add to daily log
```bash
cd ${CLAUDE_SKILL_DIR} &&node src/cli.js add --daily "<text>"
```

### Read a memory file
```bash
cd ${CLAUDE_SKILL_DIR} &&node src/cli.js get <filename>
```
Files: `MEMORY.md`, `IDENTITY.md`, `USER.md`, `daily/YYYY-MM-DD.md`

### List all memory files
```bash
cd ${CLAUDE_SKILL_DIR} &&node src/cli.js list
```

### Rebuild vector index
```bash
cd ${CLAUDE_SKILL_DIR} &&node src/cli.js reindex
```

### Check status
```bash
cd ${CLAUDE_SKILL_DIR} &&node src/cli.js status
```
