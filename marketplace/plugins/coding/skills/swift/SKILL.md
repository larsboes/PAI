...
- Working with plugin-based architectures
...
```swift
@MainActor
protocol AppPlugin: Identifiable, Observable, AnyObject {
    var id: String { get }
...
```swift
// Publish event
EventBus.shared.emit(.dataRequested, from: pluginId, payload: data)

// Subscribe to events
eventBus.on(.dataRequested) { event in
    // Handle event
}
```
...
### Build Command
```bash
xcodebuild -scheme MyApp -destination 'platform=macOS' build 2>&1 | tail -50
```

### Test Command
```bash
xcodebuild -scheme MyApp -destination 'platform=macOS' test 2>&1 | tail -50
```
