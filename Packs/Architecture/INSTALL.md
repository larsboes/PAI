...
```
Installing Architecture — Clean Architecture review/check/fix, DDD tactical/strategic patterns, hexagonal, CQRS, event sourcing
```
...
### Pi (primary)
```bash
grep -r "Architecture" ~/.pi/agent/config.yml 2>/dev/null && echo "OK config references PAI Packs dir" || echo "Check ~/.pi/agent/config.yml"
...
```bash
SKILL_DIR="$HOME/.claude/skills/architecture"
...
cp -r "$(pwd)/src/." ~/.claude/skills/architecture/
echo "Installed to ~/.claude/skills/architecture/"
```
