# DeepAnalysis — Installation Guide
...
```
Installing DeepAnalysis — Multi-dimensional comprehension of complex systems — structure, causality, hidden dependencies
```
...
### Pi (primary)
```bash
grep -r "DeepAnalysis" ~/.pi/agent/config.yml 2>/dev/null && echo "OK config references PAI Packs dir" || echo "Check ~/.pi/agent/config.yml"
...
```bash
SKILL_DIR="$HOME/.claude/skills/DeepAnalysis"
...
cp -r "$(pwd)/." ~/.claude/skills/DeepAnalysis/
echo "Installed to ~/.claude/skills/DeepAnalysis/"
```
