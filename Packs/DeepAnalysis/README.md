---
name: DeepAnalysis
pack-id: pai-deepanalysis-v1.0.0
version: 1.0.0
author: larsboes
description: Multi-dimensional comprehension of complex systems — map structure, trace causality, surface hidden dependencies, synthesize into actionable insight.
type: skill
platform: pi,claude-code
source: PAI Fork (larsboes)
---

# DeepAnalysis

Multi-dimensional comprehension of complex systems — map structure, trace causality, surface hidden dependencies, synthesize into actionable insight.

## Installation

Point your AI at this directory:

```
"Install the DeepAnalysis pack from PAI/Packs/DeepAnalysis/"
```

Your AI reads `INSTALL.md` and walks through the wizard: system check, dependency verification, installation, verification.

## What's Included

```
  SKILL.md\n  Workflows/
```

The skill source is `SKILL.md`. See `INSTALL.md` for setup and `VERIFY.md` to confirm everything works.

## Source

Original skill created for PAI Fork — not in upstream danielmiessler/Personal_AI_Infrastructure.

## License

MIT — see [PAI LICENSE](../../LICENSE).
