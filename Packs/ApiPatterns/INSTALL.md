# ApiPatterns — Installation Guide
...
```
Installing ApiPatterns — Direct HTTP API patterns for OpenAI, Anthropic, GitHub, Stripe — raw curl/fetch, skip SDKs
```
...
### Pi (primary)
```bash
grep -r "ApiPatterns" ~/.pi/agent/config.yml 2>/dev/null && echo "OK config references PAI Packs dir" || echo "Check ~/.pi/agent/config.yml"
...
```bash
SKILL_DIR="$HOME/.claude/skills/ApiPatterns"
...
cp -r "$(pwd)/." ~/.claude/skills/ApiPatterns/
echo "Installed to ~/.claude/skills/ApiPatterns/"
```
