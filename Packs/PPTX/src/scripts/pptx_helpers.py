...
Provides shape helpers, layout utilities, slide recipes, and
slide management. 
...
Usage:
    from pptx_helpers import SlideBuilder, SlideRecipes, GanttBuilder, COLORS

    sb = SlideBuilder("path/to/deck.pptx")
    recipes = SlideRecipes(sb)

    recipes.title_slide("My Presentation", "Your Name — Date")
...
class BrandColors:
    """Brand color palette — customize for your organization."""
    # Brand
...
class SlideBuilder:
    ...
    def add_numbered_circle(self, slide, x, y, size, number,
        ...
        if gradient:
            ...
            shape.fill.solid()
            shape.fill.fore_color.rgb = color or COLORS.MAGENTA
        shape.line.fill.background()
        tf = shape.text_frame
        tf.word_wrap = False
        p = tf.paragraphs[0]
    ...
    def add_info_card(self, slide, x, y, w, h, title='', body='',
        ...
        """
        bg_color = bg_color or COLORS.CARD_BG
    ...
    def add_highlight_card(self, slide, x, y, w, h, text='',
        ...
        bg_color = bg_color or COLORS.MAGENTA
        text_color = text_color or COLORS.WHITE
        card = slide.shapes.add_shape(
            MSO_SHAPE.ROUNDED_RECTANGLE, Inches(x), Inches(y), Inches(w), Inches(h))
        card.fill.solid()
    ...
    def add_process_flow(self, slide, items, x_start, y, item_w=2.3,
        ...
        for i, (title, subtitle) in enumerate(items):
            ...
            bg = COLORS.MAGENTA if is_highlight else COLORS.CARD_BG
            txt_color = COLORS.WHITE if is_highlight else COLORS.DARK
            ...
            if i < len(items) - 1:
                self.add_label(slide, x + item_w, y + item_h / 2 - 0.15,
                               gap, 0.3, '→', size=14, color=COLORS.GRAY,
    ...
    def add_footer_bar(self, slide, number=None, section='', year='',
        ...
        """
        text_color = text_color or COLORS.GRAY
        line_color = line_color or COLORS.LIGHT_GRAY
    ...
    def add_legend_item(self, slide, x, y, shape_type, color, label,
        ...
        label_color = label_color or COLORS.GRAY
        if shape_type == 'bar':
    ...
    def add_table(self, slide, x, y, w, h, data,
        ...
        """
        header_color = header_color or COLORS.MAGENTA
        header_text_color = header_text_color or COLORS.WHITE
        stripe_color = stripe_color or COLORS.SURFACE
        rows = len(data)
    ...
    def add_rich_text(self, slide, x, y, w, h, paragraphs,
        ...
        """
        default_color = default_color or COLORS.DARK
        tb = slide.shapes.add_textbox(Inches(x), Inches(y), Inches(w), Inches(h))
        tf = tb.text_frame
        tf.word_wrap = wrap
...
class SlideRecipes:
    ...
    def title_slide(self, title, subtitle='', notes=''):
        ...
        self.sb.add_rect(slide, 0, 0, self.SLIDE_W, 0.08, COLORS.MAGENTA)
        # Title — centered
        self.sb.add_label(slide, self.MARGIN, 2.2, self.CONTENT_W, 1.5,
                          title, size=40, bold=True, color=COLORS.DARK,
                          align=PP_ALIGN.CENTER)
        # Subtitle
        if subtitle:
            self.sb.add_label(slide, self.MARGIN, 3.8, self.CONTENT_W, 0.6,
                              subtitle, size=16, color=COLORS.GRAY,
                              align=PP_ALIGN.CENTER)
        if notes:
    ...
    def section_divider(self, title, subtitle='', notes=''):
        ...
        self.sb.add_rect(slide, 0, 0, 0.15, self.SLIDE_H, COLORS.MAGENTA)
        # Title
        self.sb.add_label(slide, 1.0, 2.5, 10, 1.2, title,
                          size=36, bold=True, color=COLORS.DARK)
        if subtitle:
    ...
    def content_slide(self, title, bullets, subtitle='', notes=''):
        ...
        slide = self.sb.add_blank_slide()
        # Magenta top accent
        self.sb.add_rect(slide, 0, 0, self.SLIDE_W, 0.04, COLORS.MAGENTA)
        # Title
        self.sb.add_label(slide, self.MARGIN, 0.4, self.CONTENT_W, 0.6,
                          title, size=28, bold=True, color=COLORS.DARK)
        if subtitle:
            self.sb.add_label(slide, self.MARGIN, 1.0, self.CONTENT_W, 0.4,
                              subtitle, size=12, color=COLORS.GRAY)
        # Bullet list
    ...
    def kpi_dashboard(self, metrics, title='Key Metrics', notes=''):
        ...
        for i, metric in enumerate(metrics):
            ...
            self.sb.add_label(slide, card_x, card_y + 0.4, card_w, 1.0,
                              ...
                              color=COLORS.MAGENTA, align=PP_ALIGN.CENTER)
            # Label
            self.sb.add_label(slide, card_x, card_y + 1.5, card_w, 0.4,
                              label, size=12, color=COLORS.DARK,
                              align=PP_ALIGN.CENTER)
            # Delta
            if delta:
                delta_str = str(delta)
                delta_color = (COLORS.GREEN if delta_str.startswith('+')
                               else COLORS.RED if delta_str.startswith('-') or delta_str.startswith('−')
                               else COLORS.GRAY)
    ...
    def comparison_slide(self, title, left_title, left_items,
        ...
        left_color = left_color or COLORS.TEAL
        right_color = right_color or COLORS.MAGENTA
        slide = self.sb.add_blank_slide()
        self.sb.add_rect(slide, 0, 0, self.SLIDE_W, 0.04, COLORS.MAGENTA)
        self.sb.add_label(slide, self.MARGIN, 0.4, self.CONTENT_W, 0.6,
                          title, size=28, bold=True, color=COLORS.DARK)
        ...
        for i, (col_title, items, color) in enumerate([
            ...
            self.sb.add_label(slide, col_x + 0.2, top_y + 0.08, col_w - 0.4, 0.4,
                              col_title, size=14, bold=True, color=COLORS.WHITE)
            # Items
    ...
    def agenda_slide(self, items, current_index=None, title='Agenda', notes=''):
        ...
        for i, item in enumerate(items):
            ...
            bg = COLORS.MAGENTA if is_current else COLORS.CARD_BG
            text_color = COLORS.WHITE if is_current else COLORS.DARK

            self.sb.add_rounded_rect(slide, item_x, y, item_w, item_h, bg)
            # Number circle
            self.sb.add_numbered_circle(
                slide, item_x + 0.15, y + 0.08, 0.42, str(i + 1),
                gradient=is_current, color=COLORS.GRAY if not is_current else None)
            # Text
    ...
    def image_slide(self, title, image_path, caption='',
        ...
        if caption:
            self.sb.add_label(slide, text_x, 1.2, 6.0, 4.5,
                              caption, size=13, color=COLORS.DARK, wrap=True)
        # Image
    ...
    def numbered_section_divider(self, title, number, subtitle='',
        ...
        """
        circle_color = circle_color or COLORS.MAGENTA
        slide = self.sb.add_blank_slide()
        ...
        self.sb.add_label(slide, self.MARGIN, 2.8, self.CONTENT_W, 1.5,
                          title, size=44, bold=True, color=COLORS.DARK)
    ...
    def icon_grid_slide(self, title, items, cols=3, icon_color=None, notes=''):
        ...
        """
        icon_color = icon_color or COLORS.MAGENTA
        slide = self.sb.add_blank_slide()
        self.sb.add_rect(slide, 0, 0, self.SLIDE_W, 0.04, COLORS.MAGENTA)
        self.sb.add_label(slide, self.MARGIN, 0.4, self.CONTENT_W, 0.6,
                          title, size=28, bold=True, color=COLORS.DARK)
        ...
        for i, item in enumerate(items):
            ...
            is_alt = i % 2 == 1
            c_color = COLORS.SURFACE if is_alt else icon_color
            t_color = COLORS.DARK if is_alt else COLORS.WHITE
            ...
            self.sb.add_label(slide, text_x, y + circle_size + 0.15,
                              ...
                              size=14, bold=True, color=COLORS.DARK,
                              align=PP_ALIGN.CENTER)
    ...
    def milestone_timeline_slide(self, title, milestones, notes=''):
        ...
        for i, (label, bullets) in enumerate(milestones):
            ...
            # Numbered circle
            c = self.sb.add_circle(slide, x_base, y, circle_size, COLORS.MAGENTA)
            tf = c.text_frame
            tf.word_wrap = False
            p = tf.paragraphs[0]
            p.text = str(label)
            ...
            if i < n - 1:
                ...
                self.sb.add_line(slide, line_x, y + circle_size,
                                 0.02, step_h - circle_size, COLORS.MAGENTA)
    ...
    def risk_grid_slide(self, title, risks, cols=2, notes=''):
        ...
        for i, (label, bullets) in enumerate(risks):
            ...
            # Numbered circle
            c = self.sb.add_circle(slide, x, y, circle_size, COLORS.MAGENTA)
            tf = c.text_frame
            tf.word_wrap = False
            p = tf.paragraphs[0]
            p.text = str(label)
    ...
    def phase_tree_slide(self, title, phase_number, phase_name, items,
        ...
        c = self.sb.add_circle(slide, circle_x, circle_y, circle_size, COLORS.MAGENTA)
        tf = c.text_frame
        tf.word_wrap = False
        p = tf.paragraphs[0]
        ...
        self.sb.add_label(slide, circle_x + circle_size + 0.2, circle_y + 0.1,
                          4, 0.5, phase_name,
                          size=22, bold=True, color=COLORS.DARK)
        ...
        self.sb.add_line(slide, tree_x - 0.01, circle_y + circle_size,
                         0.02, total_h + 0.3, COLORS.MAGENTA)
    ...
    def team_slide(self, title, members, notes=''):
        ...
        for i, member in enumerate(members):
            ...
            if image_path and Path(image_path).exists():
                ...
                # Placeholder circle
                colors = [COLORS.MAGENTA, COLORS.TEAL, COLORS.PURPLE,
                          COLORS.SKY_BLUE]
            ...
            self.sb.add_label(slide, text_x, name_y, col_w, 0.4,
                              name, size=16, bold=True, color=COLORS.DARK,
                              align=PP_ALIGN.CENTER)
...
class GanttBuilder:
    ...
    Usage:
        ...
        gantt.add_task("Task A", 0, 0.0, 0, 0.5, COLORS.MAGENTA)
        gantt.add_milestone("Key Date", 0, 0.5)
        gantt.add_today_marker(-0.15)
        gantt.add_legend([("Erledigt", "bar", COLORS.GREEN), ...])
    """
    ...
    def __init__(self, sb: SlideBuilder, slide,
        ...
        sb.add_label(slide, 0.5, 0.25, 6, 0.6, title,
                     size=32, bold=True, color=COLORS.DARK)
        ...
        for i, m in enumerate(months):
            ...
            if i % 2 == 1:
                sb.add_rect(slide, x, top_rows, col_w,
                            row_spacing * 25, COLORS.SURFACE)
            sb.add_label(slide, x, top_header, col_w, 0.3, m,
                         size=11, bold=True, color=COLORS.DARK,
                         align=PP_ALIGN.CENTER)
    ...
    def add_today_marker(self, month_fraction=0.0, month_index=0,
        ...
        self.sb.add_rect(self.slide, x, top, 0.02, height, COLORS.RED)
        self.sb.add_label(self.slide, x - 0.3, top - 0.25, 0.7, 0.2,
                          label, size=9, bold=True, color=COLORS.RED,
                          align=PP_ALIGN.CENTER)
    ...
    def add_slide_number(self, slide_num=None):
        ...
        self.sb.add_label(self.slide, 12.0, 7.1, 1, 0.3, str(num),
                          size=10, color=COLORS.GRAY, align=PP_ALIGN.RIGHT)
