...
| **Colors** | `COLORS` | Configurable brand color palette |
...
```python
import sys; sys.path.insert(0, str(Path.home() / ".pi/skills/pptx/scripts"))
from pptx_helpers import SlideBuilder, SlideRecipes, ChartBuilder, GanttBuilder, COLORS

sb = SlideBuilder("deck.pptx")
recipes = SlideRecipes(sb)

# High-level recipes (one call per slide)
recipes.title_slide("Q1 Results", "Your Name — Date")
...
recipes.team_slide("Team", [("Alice Smith", None, "Lead Engineer"), ...])
...
recipes.comparison_slide(title, left_title, left_items, right_title, right_items,
                         left_color=COLORS.TEAL, right_color=COLORS.MAGENTA, notes='')
...
All coordinates in **inches**. Colors default to brand palette.
...
```python
# Simple label
sb.add_label(slide, x, y, w, h, "Text", size=10, bold=False, color=COLORS.DARK,
...
sb.add_rich_text(slide, x, y, w, h, [
    ...
    {"text": "Italic note", "size": 10, "italic": True, "color": COLORS.GRAY},
])
...
sb.add_glass_card(slide, x, y, w, h, text='', text_size=12,
                  text_color=COLORS.WHITE, alpha=20,       # 0=invisible, 100=opaque
...
sb.add_table(slide, x, y, w, h, data,
             ...
             header_color=COLORS.MAGENTA,           # Header bg
             header_text_color=COLORS.WHITE,        # Header text
             stripe_color=COLORS.SURFACE)           # Alternating rows
```
...
gantt.add_task("Literature Review", 0, 0.0, 1, 0.5, COLORS.TEAL)
gantt.add_task("Prototype", 1, 0.0, 2, 0.5, COLORS.MAGENTA)
gantt.add_milestone("Mid-Review", 2, 0.0)
gantt.add_today_marker(month_fraction=0.5, month_index=0)
gantt.add_legend([("Done", "bar", COLORS.GREEN), ("Active", "bar", COLORS.MAGENTA)])
...
```python
# Brand
COLORS.MAGENTA          # E20074 — Primary brand
COLORS.MAGENTA_DARK     # 9A0050
COLORS.MAGENTA_LIGHT    # F9A8D4
COLORS.MAGENTA_10       # FCE7F3 — 10% tint

# Neutrals
COLORS.DARK             # 262626 — Primary text
COLORS.GRAY             # 6B7280 — Secondary text
COLORS.LIGHT_GRAY       # E5E5E5 — Borders
COLORS.SURFACE          # F5F5F7 — Backgrounds
COLORS.CARD_BG          # EDEDED — Card backgrounds
COLORS.WHITE            # FFFFFF

# Theme accents
COLORS.TEAL             # 32B9AF
COLORS.LIGHT_BLUE       # A4DEEE
COLORS.PEACH            # ECCCBF
COLORS.SKY_BLUE         # 00A8E6
COLORS.PURPLE           # 6E648C

# Semantic
COLORS.GREEN            # 059669 — Success
COLORS.RED              # DC2626 — Error/today
COLORS.BLUE             # 2563EB — Info
COLORS.WARNING          # D97706 — Warning
```
