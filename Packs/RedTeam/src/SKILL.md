---
name: RedTeam
description: "Adversarial analysis using parallel expert agents to stress-test ideas, strategies, and plans. Two workflows: ParallelAnalysis (attack → synthesis → steelman) and AdversarialValidation (competing proposals). Targets arguments, not systems. USE WHEN red team, attack idea, counterarguments, critique, stress test, devil's advocate, find weaknesses, break this, poke holes, what could go wrong, strongest objection, adversarial validation. NOT FOR network/system vulnerability testing (use Security). NOT FOR AI instruction auditing (use BitterPillEngineering)."
effort: high
---
