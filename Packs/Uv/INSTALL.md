...
```
Installing Uv — Python packaging with uv, project templates, dependency management, CI/CD patterns
```
...
### Pi (primary)
```bash
grep -r "Uv" ~/.pi/agent/config.yml 2>/dev/null && echo "OK config references PAI Packs dir" || echo "Check ~/.pi/agent/config.yml"
...
```bash
SKILL_DIR="$HOME/.claude/skills/uv"
...
cp -r "$(pwd)/src/." ~/.claude/skills/uv/
chmod +x ~/.claude/skills/uv/scripts/*.sh
echo "Installed to ~/.claude/skills/uv/"
```
