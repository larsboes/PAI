...
```
Installing TypeScript — TypeScript patterns, Bun runtime/tooling, and React component architecture
```
...
### Pi (primary)
```bash
grep -r "TypeScript" ~/.pi/agent/config.yml 2>/dev/null && echo "OK config references PAI Packs dir" || echo "Check ~/.pi/agent/config.yml"
...
```bash
SKILL_DIR="$HOME/.claude/skills/TypeScript"
...
cp -r "$(pwd)/src/." ~/.claude/skills/typescript/
echo "Installed to ~/.claude/skills/typescript/"
```
