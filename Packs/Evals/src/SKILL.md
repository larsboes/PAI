---
name: Evals
description: "AI agent evaluation framework — run evals, compare models/prompts, create judges, simulate multi-turn scenarios, and score with pass@k. Three grader types: code-based, model-based (LLM rubric), human. USE WHEN eval, evaluate, benchmark, regression test, compare models, compare prompts, create judge, test agent, quality check, pass@k, agent transcript, scenario simulation, capability test, before/after comparison, suite saturation, failure to task. NOT FOR general research (use Research)."
effort: high
context: fork
---
