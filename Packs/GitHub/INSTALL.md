...
```
Installing GitHub — Interact with GitHub using the gh CLI. Scripts for PR creation, review, releases, and CI management.
```
...
### Pi (primary)
```bash
grep -r "GitHub" ~/.pi/agent/config.yml 2>/dev/null && echo "OK config references PAI Packs dir" || echo "Check ~/.pi/agent/config.yml"
...
cp -r "$(pwd)/src/." ~/.claude/skills/GitHub/
echo "Installed to ~/.claude/skills/GitHub/"
```
