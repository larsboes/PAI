...
description: "Physics-based reasoning that deconstructs problems to irreducible truths — DECONSTRUCT to parts, CHALLENGE every element as hard/soft constraint or unvalidated assumption, RECONSTRUCT from fundamentals alone. USE WHEN first principles, fundamental truths, challenge assumptions, is this a real constraint, rebuild from scratch, question everything, reasoning by analogy, is this really necessary, start over, physics first. NOT FOR incident investigation (use RootCauseAnalysis). NOT FOR structural feedback loops (use SystemsThinking)."
effort: high
---
