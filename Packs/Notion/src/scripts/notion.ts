...
import type { Page, Database } from "../references/notion-types.js";
...
async function readCommand(args: string[]) {
  if (args.length === 0) {
    console.error("Usage: notion read <page-id-or-url> [--shallow] [--download=<dir>]");
    process.exit(1);
  }
  ...
  const blocks = await fetchBlocksRecursively(pageId, !shallow);
...
async function searchCommand(args: string[]) {
  ...
  for (const item of results) {
    const icon = item.object === "page" ? "[Page]" : "[Database]";
...
COMMANDS:
  read <page-id-or-url>      Export a page to Markdown
    --shallow                Skip recursive child block fetch
