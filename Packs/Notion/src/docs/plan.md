...
- [x] Task 3: Create `lib/notion-md.ts` — Block to Markdown converter (paragraph, heading, list, code)
...
## Notes
- Types are comprehensive (400+ lines covering all block types, properties, and responses).
- Client has exponential backoff for rate limits.
- Markdown converter handles all common block types including tables, columns, and callouts.
- CLI supports: read (with --children), query (with --filter), search, create, update, schema, export.
