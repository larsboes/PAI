  ...
  switch (block.type) {
    ...
    case "file": {
      ...
      const caption = block.file.caption?.length
        ...
        : "File";
      return `${indent}[File: ${caption}](${url})`;
    }
    ...
    case "child_page":
      return block.child_page
        ? `${indent}[Page: ${block.child_page.title}](https://notion.so/${block.id.replace(/-/g, "")})`
        : "";

    case "child_database":
      return block.child_database
        ? `${indent}[Database: ${block.child_database.title}](https://notion.so/${block.id.replace(/-/g, "")})`
        : "";
...
// ============================================================================
// Column layout (best-effort Markdown rendering)
// ============================================================================

/**
 * Render a column_list as sequential sections separated by blank lines.
 * Markdown doesn't natively support columns, so we render content sequentially.
 */
