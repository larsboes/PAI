...
def run_obsidian_cli(args: list[str]) -> str | None:
    if not OBSIDIAN_BIN or not os.path.exists(OBSIDIAN_BIN):
        return None
    try:
...
def cmd_daily(args) -> None:
    ...
    if path_rel:
        ...
        # Fallback: common daily note patterns
        today = datetime.date.today().strftime("%Y-%m-%d")
        # Try common locations
        candidates = [
            f"Daily Notes/{today}.md",
            f"Journal/{today}.md",
            f"Journal/Daily/{today}.md",
        ]
        for candidate in candidates:
            full = os.path.join(vault, candidate)
            if os.path.exists(full):
                print(full)
                return
        # Default fallback
        print(f"{vault}/Daily Notes/{today}.md")
...
def cmd_active(args) -> None:
    ...
    if not output:
        print("Could not get active file (Obsidian might not be running or OBSIDIAN_BIN not set).")
        return
    data = {}
...
def cmd_open(args) -> None:
    ...
    result = run_obsidian_cli(["open", f"path={path}"])
    if result is None:
        print("Cannot open file: OBSIDIAN_BIN not configured or not found.")
        return
    print(f"Opened {path}")
...
def cmd_health(args) -> None:
    ...
    if broken_links:
        seen: set = set()
        print(f"\n[WARN] Broken Wikilinks ({len(broken_links)} found)")
    ...
    if dups:
        print(f"\n[WARN] Duplicate Titles ({len(dups)} found)")
