...
```
Installing OSINT — Structured OSINT investigations — people, companies, domains, investments, threat intel
```
...
### Pi (primary)
```bash
grep -r "OSINT" ~/.pi/agent/config.yml 2>/dev/null && echo "OK config references PAI Packs dir" || echo "Check ~/.pi/agent/config.yml"
...
```bash
SKILL_DIR="$HOME/.claude/skills/OSINT"
...
cp -r "$(pwd)/." ~/.claude/skills/OSINT/
echo "Installed to ~/.claude/skills/OSINT/"
```
