...
# Directories
PI_SKILLS="${HOME}/.pai/skills"
PUBLIC_SKILLS="${PUBLIC_SKILLS:-./skills}"
...
scan_for_pii() {
    ...
    # Check for absolute paths (excluding github.com URLs which are public)
    if grep "~" "$file" 2>/dev/null | grep -v "github.com/" > /dev/null; then
    ...
    local github_count=$(grep -o "github.com/{{username}}" "$file" 2>/dev/null | wc -l | tr -d ' ')
    ...
    if grep -qiE "personal_marker_1|personal_marker_2|personal_marker_3|personal_marker_4" "$file" 2>/dev/null; then
        issues="${issues}personal_context;"
    fi
    
    # Check for personal name as subject (indicates personalized skill)
    if grep -q "When {{name}}\|Let {{name}}\|{{name}} says\|{{name}} can\|{{name}} wants" "$file" 2>/dev/null; then
