---
name: ISA
description: "Scaffold, interview, check, reconcile, and seed Ideal State Artifacts — the commitment-time document that articulates the ideal state of any project or task, drives the build, and records understanding evolution. Auto-invoked by the Algorithm at OBSERVE for non-trivial tasks. USE WHEN ideal state, ISA, ISC, ideal state criteria, scaffold ISA, interview for ISA, check ISA completeness, reconcile feature file, seed from repo, project specification, articulating done. NOT FOR creating new skills (use CreateSkill), algorithm execution, or retrospective write-ups."
effort: medium
---
