...
- **v6.4.0** (2026-06-09) — **TELOS-aware ideal state release.** Field gap: the Algorithm articulated ideal-state criteria with zero reference to the principal's TELOS (MISSION/GOALS/BELIEFS/CHALLENGES) — `grep -c telos v6.3.0.md` = 0. Worse, the TELOS never even reached context: CLAUDE.md loaded it via `@${VAULT_PATH}/…` imports, but Claude Code does not expand env vars in `@import` paths, so IDENTITY/SOUL/TELOS/PERSONAL_CONTEXT silently failed to load — {{DA_NAME}} planned without knowing {{PRINCIPAL_NAME}}'s identity or goals. Two-part fix: (1) **runtime loader** — new `hooks/LoadTelos.hook.ts` reads `$VAULT_PATH` at SessionStart and injects the TELOS files as a `<system-reminder>` (env var works in a hook; loader is public/repo-tracked, content stays private in the vault; portable across harnesses). The 4 personal `@import` lines were removed from CLAUDE.md. (2) **doctrine** — a new `Ideal state is TELOS-relative` paragraph in the preamble, a `TELOS alignment pass` sub-step in OBSERVE before the ISC quality gates (emits `🧭 TELOS ALIGNMENT: [goal served | N/A — mechanical task]`), and a soft `TELOS alignment` ISC quality gate. Break-3 repoint: the LEARN router `identity` target moved from the nonexistent `USER/PRINCIPAL_IDENTITY.md` / `USER/DA_IDENTITY.md` to the private vault (`$VAULT_PATH/Atlas/TELOS/IDENTITY.md`, `Atlas/Personal/PERSONAL_CONTEXT.md`); `PAI_SYSTEM_PROMPT.md` persona refs repointed likewise. C/R/L: conjectured: pointing CLAUDE.md `@import` at `${VAULT_PATH}` would load the principal's TELOS, and an ideal-state engine would implicitly serve the principal's goals. refuted_by: `@import` does not interpolate env vars (verified vs code.claude.com/docs/en/memory) so TELOS loaded in zero sessions; and with no doctrinal instruction to consult it, ISC sets optimized the literal request while ignoring stated beliefs/challenges. learned: personal context must be injected by a hook that reads the env var at runtime, not by a static `@import`; and TELOS alignment must be an explicit OBSERVE step with an output token + gate, or it doesn't happen. criterion_now: LoadTelos injects TELOS every session; OBSERVE emits `🧭 TELOS ALIGNMENT` and the soft gate checks it; identity learnings route to the private vault. Migration: `echo 6.4.0 > LATEST` (single source; no settings.json field per v6.2.0 cleanup) + register SessionStart→LoadTelos in settings.json. Rollback: `echo 6.3.0 > LATEST` (TELOS still loads via the hook independently of doctrine version). v6.4.x deferred: a hook that audits the `🧭 TELOS ALIGNMENT` token is present at OBSERVE→THINK; TELOS-goal tagging in reflection JSONL.
...
v6.4.0 closes the gap between the Algorithm's "ideal state" engine and the principal it serves. The frame already said the experiential metric is *euphoric surprise on convergence on what the user actually wanted* — but "what the user actually wanted" was never anchored to anything durable. The principal's TELOS (mission, goals, beliefs, challenges) lived in the private Obsidian vault, and the only bridge into context was a CLAUDE.md `@import` that silently did nothing.

**The two conjectures that broke, in C/R/L form:**

- **conjectured:** `@${VAULT_PATH}/Atlas/TELOS/IDENTITY.md` in CLAUDE.md would load the principal's identity/soul/goals into every session.
  **refuted by:** Claude Code's `@import` grammar supports only relative / absolute / `~/` paths — it does not interpolate `${VAR}`/`$VAR` (confirmed against the official memory docs). The `@` consumed the literal characters `${VAULT_PATH}`, which is not a path on disk, so all four imports resolved to nothing. The empirical tell: TELOS content never appeared in any session's context. {{DA_NAME}} had been running without {{PRINCIPAL_NAME}}'s identity, voice, or goals the entire time.
  **learned:** private, machine-relative context must be injected by a **hook that reads the environment variable at runtime** (`process.env.VAULT_PATH`), not by a static `@import`. This also keeps the loader public/repo-tracked while the content stays private in the vault, and makes the mechanism portable to any harness with a SessionStart hook.
  **criterion now:** `hooks/LoadTelos.hook.ts` injects TELOS each session from `$VAULT_PATH`; the personal `@import` lines are gone from CLAUDE.md.

- **conjectured:** an ideal-state engine would, by construction, serve the principal's actual goals — alignment is implicit in "what the user wanted."
  **refuted by:** with no doctrinal instruction to consult TELOS (and TELOS absent from context anyway), ISC sets optimized the literal request. Nothing made the executor ask "which GOAL does this advance? does any criterion cut against a stated BELIEF or ignore a known CHALLENGE?" Implicit alignment is no alignment.
  **learned:** alignment must be an explicit step with an output token and a gate, or it doesn't happen — the same lesson v6.3.0 learned about thinking-capability names (open intentions get wallpapered under tier pressure; closed/explicit checks survive).
  **criterion now:** the preamble declares ideal state TELOS-relative; OBSERVE runs a TELOS alignment pass emitting `🧭 TELOS ALIGNMENT: [goal served | N/A — mechanical task]`; a soft ISC quality gate verifies the pass ran and that any belief/challenge tension is encoded as an ISC/anti-criterion rather than left as prose.

**Break-3 repoint.** The LEARN router's `identity` write-target and `PAI_SYSTEM_PROMPT.md`'s persona references pointed at `USER/PRINCIPAL_IDENTITY.md` / `USER/DA_IDENTITY.md`, which do not exist in this fork — identity was relocated to the vault. Both now point at `$VAULT_PATH/Atlas/TELOS/IDENTITY.md` (DA persona) and `Atlas/Personal/PERSONAL_CONTEXT.md` (principal). Deliberate divergence from upstream's `USER/*_IDENTITY.md` convention; documented in `ADDITIONS.md §6`.

Migration: `echo 6.4.0 > LATEST`; register `SessionStart → LoadTelos.hook.ts` in `settings.json`. Rollback: `echo 6.3.0 > LATEST` — TELOS still loads (the hook is independent of doctrine version); only the alignment pass/gate revert.
